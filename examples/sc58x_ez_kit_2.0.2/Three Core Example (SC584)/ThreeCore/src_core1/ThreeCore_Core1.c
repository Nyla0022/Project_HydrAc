...
/**
 * Initialize the LED that we use for Core 1
 */
static ADI_GPIO_RESULT initialize_led(void);
static ADI_GPIO_RESULT initialize_led(void)
{
    return adi_gpio_SetDirection(CORE1_LED_PORT, CORE1_LED_PIN, ADI_GPIO_DIRECTION_OUTPUT);
}
...
/**
 * Initialize the Push Button that we use for Core 1
 */
...
static ADI_GPIO_RESULT initialize_push_button(void)
{
    ...
    {
        result = adi_gpio_SetDirection(CORE1_PB_PORT, CORE1_PB_PORT_PIN, ADI_GPIO_DIRECTION_INPUT);
    }
    if (result == ADI_GPIO_SUCCESS)
    {
        /* Set edge sense mode  */
        result = adi_gpio_SetPinIntEdgeSense(CORE1_PB_PINT, CORE1_PB_PINT_PIN, ADI_GPIO_SENSE_RISING_EDGE);
    }
    if (result == ADI_GPIO_SUCCESS)
    {
        /* Register pinint callback */
        result = adi_gpio_RegisterCallback(CORE1_PB_PINT, CORE1_PB_PINT_PIN, pintCallback, (void*)0);
    }
    if (result == ADI_GPIO_SUCCESS)
    {
        /* Assign pin interrupt  */
        result = adi_gpio_PinInterruptAssignment(CORE1_PB_PINT, CORE1_PIN_ASSIGN_BYTE, CORE1_PIN_ASSIGN);
    }
    if (result == ADI_GPIO_SUCCESS)
    {
        /* Set pin interrupt mask */
        result = adi_gpio_EnablePinInterruptMask(CORE1_PB_PINT, CORE1_PB_PINT_PIN, true);
    }
    return result;
}
...
static void blink_led_and_wait_for_button(void)
{
    ...
    {
        adi_gpio_Toggle(CORE1_LED_PORT, CORE1_LED_PIN);
        delay(500u);
        count--;
    }
    adi_gpio_Set(CORE1_LED_PORT, CORE1_LED_PIN);
...
int main(void)
{
    ...
    */
    print("Core 1 received message from Core 0\n");
    ...
    */
    adi_gpio_Clear(CORE1_LED_PORT, CORE1_LED_PIN);
    shut_down_comms();
